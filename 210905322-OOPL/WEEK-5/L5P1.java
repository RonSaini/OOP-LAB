...
class Student
{
  ...
  int month;
  int dayOfMonth;
  GregorianCalendar doj = new GregorianCalendar(year,month,dayOfMonth);
...
public Student(int r, String name, int y, int m, int d, short s, float g, float c)
{
  rgno = r;
  this.name = name;
  year = y;
...
System.out.println("Date of Joining : "+ doj.get(Calendar.MONTH) +" "+ doj.get(Calendar.DATE) + ", "+ doj.get(Calendar.YEAR));
...
if(n<5)
{
System.out.println("Enter more than 5 records");
System.exit(0);
}
...
{
s[i] = new Student();
System.out.println("Enter the Student Records of the Student "+(i+1));
